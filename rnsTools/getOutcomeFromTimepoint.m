...
% options: type = {'nearest', 'interpolate', 'locf'}, (nearset is default)
...
%
% Example:
% dt_ref = [patients_Penn.stimOnDate];
% outcomeCellPairs = {patients_Penn.outcome};
% outcome_years = [1,2,3];
% [outcomes, dates, all_outcomes, all_dates] = getOutcomeFromTimepoint(dt_ref, outcomeCellPairs, outcome_years); 
% [outcomes, dates] = getOutcomeFromTimepoint(dt_ref, outcomeCellPairs, outcome_years, 'type', 'interpolate'); 


arguments
    dt_ref datetime
    outcomeCellPair cell
    targ_yr
    options.type {ismember(options.type, {'nearest', 'interpolate', 'locf'})} = 'nearest';
end
...
for i=1:nSets
    ...
    if isempty(outcomeCellPair{i}{1}), continue, end
    ...
    elseif isa(pt_outs, 'char')
        if strcmpi(pt_outs,'No Change'), outs= 0;
        elseif strcmpi(pt_outs,'Increased'), outs= -10;
        ...
        for m=1:length(mtch)    
            if isempty(mtch) || isempty(mtch{m})
                if strcmpi(pt_outs{m},'No Change'), outs(m)= 0;
                elseif strcmpi(pt_outs{m},'Increased'), outs(m)= -10;
    ...
    % Get years b/w implant and measurement, and closest date to target 
        ...
        elseif strcmp(options.type, 'nearest') % Take closest outcome value to date
